...
\begin{subfigure}[b]{0.49\textwidth}
  \centering\includegraphics[width=\textwidth]{figures/drop_none.png}
  \caption{No wind estimate. Miss distance $7.33$~m.}
  \label{fig:drop-none}
\end{subfigure}\hfill
\begin{subfigure}[b]{0.49\textwidth}
  \centering\includegraphics[width=\textwidth]{figures/drop_ekf.png}
  \caption{EKF estimate. Miss distance $3.64$~m.}
  \label{fig:drop-ekf}
\end{subfigure}
\caption{One descent, seen two ways, from the interactive model. The upper panel
of each pair looks straight down; the lower panel is the same flight from the
side, showing the canopy gliding forward as it descends at a constant rate. The
helicopter marks the release point and the cross marks the target. Both panels
use identical axes, and both are the same drop --- wind $2.0$~m\,s$^{-1}$ from
the south, release height $30$~m, wing loading $1.2$~kg\,m$^{-2}$, magnetometer
bias $+10^{\circ}$ --- differing only in whether the guidance estimates the wind.
These are single flights shown to make the trajectory concrete; the statistical
comparison is in \cref{tab:results}.}
\label{fig:drop}
\end{figure}
