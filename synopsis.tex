...
\definecolor{codegrey}{HTML}{6E6A62}
\definecolor{coderule}{HTML}{D8D2C2}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\footnotesize\textsc{Airdrop Trajectory Optimisation for Disaster
Relief}}
...
\begin{lstlisting}[caption={Path planning, from
\texttt{model/dubins.py}. An infinite question --- which curve is best? ---
becomes picking the smallest of six numbers. A shape that cannot connect the two
configurations returns \texttt{None} and is skipped.},label=lst:dubins]
def solve(start, end, R, only=None):
    """Shortest Dubins path: try all six shapes, keep the shortest."""
    d = np.hypot(x1 - x0, y1 - y0) / R       # normalise into units of R
    phi = np.arctan2(y1 - y0, x1 - x0)
    a, b = _mod2pi(th0 - phi), _mod2pi(th1 - phi)

    best = None
    for name in (only or TYPES):             # LSL LSR RSL RSR RLR LRL
        out = _SOLVERS[name](a, b, d)
        if out is None:                      # cannot connect these two
            continue
        t, p, q = out
        length = (t + p + q) * R
        if best is None or length < best["length"]:
            best = {"type": name, "length": length, "R": R}
    return best
\end{lstlisting}
...
\begin{lstlisting}[caption={The control law, from
\texttt{model/guidance.py}. \texttt{d\_eff} is the Dubins length rather than the
straight-line distance, so the turn needed to line up is charged to the budget.
Using the straight line made the vehicle believe it had height to spare, and it
landed short.},label=lst:weave]
ratio = d_eff / budget          # Dubins length, not straight-line distance

if ratio < np.cos(self.alpha_max):
    # ORBIT: too high even for the steepest weave, so make no net progress
    radial_err = np.clip((d - R) / max(R, 1e-6), -1.0, 1.0)
    desired = bearing + self.side * (np.pi/2.0 - radial_err * np.pi/2.0)
else:
    # WEAVE: cos(alpha) = d / budget, alternating side to side
    alpha = np.arccos(np.clip(ratio, -1.0, 1.0))
    err = _wrap(th - bearing)
    if self.side > 0 and err > alpha:     self.side = -1.0
    elif self.side < 0 and err < -alpha:  self.side = 1.0
...
\begin{lstlisting}[caption={The measurement update, from
\texttt{model/ekf.py}. The bias $b$ sits inside the heading trigonometry, which
is what makes the problem nonlinear and the filter an \emph{extended} one; with
$b$ known the same problem would be linear.},label=lst:ekf]
wx, wy, va, b = self.x
th = heading_meas + b                      # b is the compass bias: the
c, s = np.cos(th), np.sin(th)              # nonlinearity that forces an EKF

h = np.array([va * c + wx, va * s + wy])   # predicted ground velocity
H = np.array([[1.0, 0.0, c, -va * s],      # Jacobian dh/dx
              [0.0, 1.0, s,  va * c]])
